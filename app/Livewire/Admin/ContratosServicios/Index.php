...
class Index extends Component
{
    ...
    protected function applySearch(Builder $query): Builder
    {
        ...
        return $query->where(function (Builder $qq) use ($term, $tokens) {
            $qq->where('contratos.folio_contrato', 'like', "%{$term}%")
                ->orWhere('contratos.servicio_tipo', 'like', "%{$term}%")
    ...
    {
        if (filled($this->servicio)) {
            $query->where('contratos.servicio_tipo', $this->servicio);
        }
    ...
    public function exportarExcel()
    {
        ...
        $filename = 'contratos_servicios_'.now()->format('Ymd_His').'.xlsx';
